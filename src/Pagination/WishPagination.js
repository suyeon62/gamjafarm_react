...
const WishPagination = ({ getWishList }) => {
  const pageInfo = useSelector((state) => state.wish.pageInfo) || { currentPage: 1 };
  const pageNumbers = [];
    ...
    <nav aria-label="..." style={{ display: 'flex', justifyContent: 'center' }}>
      <ul style={{ listStyle: 'none', display: 'flex', margin: '0', padding: '10px' }}>
        ...
        <li style={{ pointerEvents: pageInfo.startPage <= 1 ? 'none' : 'auto' }}>
          <span
            className="page-link"
            onClick={() => getWishList(pageInfo.startPage - pageInfo.blockPage)}
            style={{ ...defaultLinkStyle, color: 'white', borderColor: '#212529' }}
          >
            &laquo;
          </span>
        </li>
